...
func TestEnvvar_Prefixes(t *testing.T) {
	...
	os.Setenv("EASY_ADMIN_CORE_REGISTRY", "mdns")

	var prefixtests = []struct {
		prefixOpts   []source.Option
		expectedKeys []string
	}{
		{[]source.Option{WithPrefix("APP", "EASY_ADMIN_CORE")}, []string{"app", "easy_admin_core"}},
		{[]source.Option{WithPrefix("EASY_ADMIN_CORE"), WithStrippedPrefix("APP")}, []string{"database", "easy_admin_core"}},
		{[]source.Option{WithPrefix("EASY_ADMIN_CORE"), WithStrippedPrefix("APP")}, []string{"database", "easy_admin_core"}},
	}
...
func TestEnvvar_WatchNextNoOpsUntilStop(t *testing.T) {
	src := NewSource(WithStrippedPrefix("EASY_ADMIN_CORE_"))
	w, err := src.Watch()
	if err != nil {
		t.Error(err)
	}
